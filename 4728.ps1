# Аудит глобальной группы безопасности - добавление пользователя
# $args[0] - token
# $args[1] - chat id
# $args[2] - ip adress or domain name, if you want run script on local machine, use $env:COMPUTERNAME
# Example: \\server.local\audit\4728.ps1 1234567890:ABC-DEF1234ghIkl-zyx57W2v1u123ew11 -1234567890 $env:COMPUTERNAME
...
for ($i = 0; $i -lt $event2.Count; $i++)
{
    ...
    $eventtextuser = (Get-ADUser -Identity $event2array[$i].'SID-пользователя').Name
    $eventtextadmin = $event2array[$i].Администратор
    $text.Add("Группа безопасности: $eventtextgroup%0AДобавлен пользователь: $eventtextuser%0AАдминистратором: $eventtextadmin%0AСервер: $server%0A---------------%0A")
    }
}
