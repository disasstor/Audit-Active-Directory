# Аудит глобальной группы безопасности - удаление пользователя
# $args[0] - token
# $args[1] - chat id
...
for ($i = 0; $i -lt $event2.Count; $i++)
{
    ...
    $text.Add("Группа безопасности: $eventtextgroup%0AУдален пользователь: $eventtextuser%0AАдминистратором: $eventtextadmin%0AСервер: $server%0A---------------%0A")
    }
}
