# Аудит локальной группы безопасности - добавление пользователя
# $args[0] - token
# $args[1] - chat id
...
for ($i = 0; $i -lt $event2.Count; $i++)
{
    ...
    $text.Add("Локальная группа безопасности: $eventtextgroup%0AДобавлен пользователь: $eventtextuser%0AАдминистратором: $eventtextadmin%0AСервер: $server%0A---------------%0A")
    }
}
